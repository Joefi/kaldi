...
import libs.nnet3.online_adaptation.common as common_train_lib
import libs.common as common_lib
import libs.nnet3.online_adaptation.chain.acoustic_model as chain_lib
...
def get_args():
    ...
    parser.add_argument("--output-name", type=str, default="output", help="output name used to compute accuracy")
...
def train(args, run_opts):
    ...
    if not os.path.exists('{0}/cegs.1.ark'.format(egs_dir)):
        if (not os.path.exists('{0}/cegs.output.1.ark'.format(egs_dir))):
            raise Exception('Neither {0}/cegs.1.ark nor {0}/cegs.output.1.ark.'
    ...
    [report, times, data] = nnet3_log_parse.generate_acc_logprob_report(
        args.dir, "log-probability", output=args.output_name)
    if args.email is not None:
