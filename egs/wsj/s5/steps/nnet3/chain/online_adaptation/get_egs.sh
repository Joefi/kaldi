...
if ! $online_cmvn; then
  ...
  feats="ark,s,cs:cat $sdata/JOB/feats.scp | apply-cmvn $cmvn_opts --utt2spk=ark:$sdata/JOB/utt2spk scp:$sdata/JOB/cmvn.scp scp:- ark:- |"
else
  ...
  feats="ark,s,cs:cat $sdata/JOB/feats.scp | apply-cmvn-online $cmvn_opts --spk2utt=ark:$sdata/JOB/spk2utt $dir/global_cmvn.stats scp:- ark:- |"
fi
...
if [ -f $dir/.error ]; then
  echo "$0: Error detected while creating train egs" && exit 1
fi
