...
def subset_egs(src_egs_dir, tar_egs_dir, srand_seed, refer_egs):
    common_lib.execute_command(
        """steps/nnet3/chain_adapt/subset_egs_random.sh --refer_egs "{refer_egs}" --srand_seed {srand_seed} {src_egs_dir} {tar_egs_dir}""".format(
            refer_egs=refer_egs,
            srand_seed=srand_seed,
            src_egs_dir=src_egs_dir,
            tar_egs_dir=tar_egs_dir))


def generate_combine_egs(egs_1_dir, egs_2_dir, output_egs_dir, lang2weight, lang2copies, num, archives, run_opts):
    common_lib.execute_command(
        """steps/nnet3/chain/multilingual/combine_egs.sh --cmd "{command}" \
                --lang2weight "{lang2weight}" \
                --lang2num-copies "{lang2copies}" \
                --archives {archives} \
                {num} {egs_1_dir} {egs_2_dir} {output_egs_dir}""".format(
                command=run_opts.egs_command,
                lang2weight=lang2weight,
                lang2copies=lang2copies,
                num=num,
                archives=archives,
                egs_1_dir=egs_1_dir,
                egs_2_dir=egs_2_dir,
                output_egs_dir=output_egs_dir))
...
def generate_chain_egs(dir, data, lat_dir, egs_dir,
    ...
    common_lib.execute_command(
        """steps/nnet3/adaptation/get_egs.sh {egs_opts} \
                --cmd "{command}" \
                --cmvn-opts "{cmvn_opts}" \
...
def train_new_models(dir, iter, srand, num_jobs,
                     ...
                     raw_model_string, si_raw_model_string, egs_dir,
                     apply_deriv_weights,
                     min_deriv_time, max_deriv_time_relative,
                     l2_regularize, l2_adapt_regularize, xent_regularize, leaky_hmm_coefficient,
    ...
    for job in range(1, num_jobs+1):
        ...
        thread = common_lib.background_command(
            """{command} {train_queue_opt} {dir}/log/train.{iter}.{job}.log \
                    nnet3-chain-adaptation {parallel_train_opts} {verbose_opt} \
                    --apply-deriv-weights={app_deriv_wts} \
                    --l2-regularize={l2} --l2-adapt-regularize={l2_adapt_regularize} \
                    --leaky-hmm-coefficient={leaky} \
                    ...
                    --srand={srand} \
                    "{raw_model}" "{si_raw_model}" {dir}/den.fst \
                    ...
                    {dir}/{next_iter}.{job}.raw""".format(
                        ...
                        l2_adapt_regularize=l2_adapt_regularize,
                        ...
                        si_raw_model=si_raw_model_string,
...
def train_one_iteration(si_model, dir, iter, srand, egs_dir,
                        ...
                        l2_regularize, l2_adapt_regularize, xent_regularize,
    ...
    train_new_models(dir=dir, iter=iter, srand=srand, num_jobs=num_jobs,
                     ...
                     si_raw_model_string=si_raw_model_string,
                     egs_dir=egs_dir,
