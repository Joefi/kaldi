...
#include "nnet3/nnet-chain-adapting-diagnostics.h"
#include "nnet3/nnet-utils.h"

namespace kaldi {
namespace nnet3 {

NnetChainAdaptComputeProb::NnetChainAdaptComputeProb(
    const NnetComputeProbOptions &nnet_config,
    const chain::ChainAdaptingOptions &chain_config,
...
const Nnet &NnetChainAdaptComputeProb::GetDeriv() const {
  if (!nnet_config_.compute_deriv)
...
void NnetChainAdaptComputeProb::Reset() {
  num_minibatches_processed_ = 0;
  objf_info_.clear();
  if (deriv_nnet_) {
    ScaleNnet(0.0, deriv_nnet_);
...
void NnetChainAdaptComputeProb::Compute(const NnetChainExample &chain_eg) {
  ...
  bool use_yent_regularization = (chain_config_.yent_regularize != 0.0),
      use_yent_derivative = false;
  GetChainComputationRequest(nnet_, chain_eg, need_model_derivative,
                             store_component_stats, use_xent_regularization, use_yent_regularization,
                             use_xent_derivative, use_yent_derivative, &request);
...
void NnetChainAdaptComputeProb::ProcessOutputs(const NnetChainExample &eg,
  ...
  for (; iter != end; ++iter) {
    ...
    bool use_yent = (chain_config_.yent_regularize != 0.0);
    if (use_yent) {
      std::string yent_name = sup.name + "-yent";
      ChainObjectiveInfo &yent_totals = objf_info_[yent_name];
      const CuMatrixBase<BaseFloat> &yent_output = computer->GetOutput(
          yent_name);
      const SparseMatrix<BaseFloat>& phone_post = sup.supervision.phone_post.GetSparseMatrix();
      CuSparseMatrix<BaseFloat> cu_phone_post(phone_post);
      CuMatrix<BaseFloat> yent_output_deriv(yent_output.NumRows(), yent_output.NumCols(),
            kUndefined);
      cu_phone_post.CopyToMat(&yent_output_deriv);

      BaseFloat yent_objf = TraceMatSmat(yent_output, cu_phone_post, kTrans);
      BaseFloat yent_weight = cu_phone_post.Sum();
      yent_totals.tot_like += yent_objf;
      yent_totals.tot_weight += yent_weight;

      if (nnet_config_.compute_deriv){
        computer->AcceptInput(yent_name, &yent_output_deriv);
      }
    }
...
bool NnetChainAdaptComputeProb::PrintTotalStats() const {
  bool ans = false;
...
const ChainObjectiveInfo* NnetChainAdaptComputeProb::GetObjective(
...
double NnetChainAdaptComputeProb::GetTotalObjective(double *total_weight) const {
...
static bool HasYentOutputs(const Nnet &nnet) {
  const std::vector<std::string> node_names = nnet.GetNodeNames();
  for (std::vector<std::string>::const_iterator it = node_names.begin();
        it != node_names.end(); ++it) {
    int32 node_index = nnet.GetNodeIndex(*it);
    if (nnet.IsOutputNode(node_index) && 
        it->find("-yent") != std::string::npos) {
      return true;
    }
  }
  return false;
...
void RecomputeStats(const std::vector<NnetChainExample> &egs,
                    const chain::ChainAdaptingOptions &chain_config_in,
                    const fst::StdVectorFst &den_fst,
                    Nnet *nnet) {
  KALDI_LOG << "Recomputing stats on nnet (affects batch-norm)";
  chain::ChainAdaptingOptions chain_config(chain_config_in);
  ...
  if (HasXentOutputs(*nnet) &&
      chain_config.yent_regularize == 0) {
    // this forces it to compute the output for xent outputs, 
    // usually 'output-xent', which
    // means that we'll be computing batch-norm stats for any
    // components in that branch that have batch-norm.
    chain_config.yent_regularize = 0.1;
  }

  ZeroComponentStats(nnet);
  NnetComputeProbOptions nnet_config;
  nnet_config.store_component_stats = true;
  NnetChainAdaptComputeProb prob_computer(nnet_config, chain_config, den_fst, nnet);
