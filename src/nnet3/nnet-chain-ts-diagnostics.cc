...
#include "nnet3/nnet-chain-ts-diagnostics.h"
...
namespace kaldi {
    namespace nnet3 {
        ...
        NnetChainTSComputeProb::NnetChainTSComputeProb(
            const NnetComputeProbOptions& nnet_config,
            const chain::ChainTSTrainingOptions& chain_config,
        ...
        const Nnet& NnetChainTSComputeProb::GetDeriv() const {
            if (!nnet_config_.compute_deriv)
        ...
        void NnetChainTSComputeProb::Reset() {
            num_minibatches_processed_ = 0;
            objf_info_.clear();
            if (deriv_nnet_) {
                ScaleNnet(0.0, deriv_nnet_);
        ...
        void NnetChainTSComputeProb::Compute(const NnetChainExample& chain_eg) {
        ...
        void NnetChainTSComputeProb::ProcessOutputs(const NnetChainExample& eg,
            ...
            for (; iter != end; ++iter) {
                ...
                chain::ChainTSTrainingOptions chain_config_copy(chain_config_);
                {
                ...
                ChainTSObjectiveInfo& totals = objf_info_[sup.name];
                ...
                if (use_xent) {
                    ChainTSObjectiveInfo& xent_totals = objf_info_[xent_name];
        ...
        bool NnetChainTSComputeProb::PrintTotalStats() const {
            bool ans = false;
            unordered_map<std::string, ChainTSObjectiveInfo, StringHasher>::const_iterator
                iter, end;
            ...
            for (; iter != end; ++iter) {
                ...
                const ChainTSObjectiveInfo& info = iter->second;
        ...
        const ChainTSObjectiveInfo* NnetChainTSComputeProb::GetObjective(
        ...
        double NnetChainTSComputeProb::GetTotalObjective(double* total_weight) const {
        ...
        void RecomputeStats(const std::vector<NnetChainExample>& egs,
            const chain::ChainTSTrainingOptions& chain_config_in,
            const fst::StdVectorFst& den_fst,
            Nnet* nnet) {
            KALDI_LOG << "Recomputing stats on nnet (affects batch-norm)";
            chain::ChainTSTrainingOptions chain_config(chain_config_in);
            ...
            NnetChainTSComputeProb prob_computer(nnet_config, chain_config, den_fst, nnet);
