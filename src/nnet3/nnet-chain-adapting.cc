...
#include "nnet3/nnet-chain-adapting.h"
#include "nnet3/nnet-utils.h"

namespace kaldi {
namespace nnet3 {

NnetChainAdapter::NnetChainAdapter(const NnetChainAdaptingOptions &opts,
...
void NnetChainAdapter::Train(const NnetChainExample &chain_eg) {
  NVTX_RANGE(__func__);
  bool need_model_derivative = true;
  const NnetTrainerOptions &nnet_config = opts_.nnet_config;
  bool use_xent_regularization = (opts_.chain_config.xent_regularize != 0.0);
  bool use_yent_regularization = (opts_.chain_config.yent_regularize != 0.0);
  ComputationRequest request;
  GetChainComputationRequest(*nnet_, chain_eg, need_model_derivative,
                             nnet_config.store_component_stats,
                             use_xent_regularization, use_yent_regularization, need_model_derivative,
                             need_model_derivative,, &request);
...
void NnetChainAdapter::TrainInternal(const NnetChainExample &eg,
...
void NnetChainAdapter::TrainInternalBackstitch(const NnetChainExample &eg,
                                               const NnetComputation &computation,
...
void NnetChainAdapter::ProcessOutputs(bool is_backstitch_step2,
  ...
  for (; iter != end; ++iter) {
    ...
    bool use_yent = (opts_.chain_config.yent_regularize != 0.0);
    if (use_yent) {
      const CuMatrixBase<BaseFloat>& yent_output = computer->GetOutput(yent_name);
      ...
      BaseFloat yent_tot_weight = cu_phone_post.Sum();
      BaseFloat yent_tot_objf = TraceMatSmat(yent_output, cu_phone_post, kTrans);
      CuMatrix<BaseFloat> yent_output_deriv(yent_output.NumRows(), yent_output.NumCols(), kUndefined);
      cu_phone_post.CopyToMat(&yent_output_deriv);
      yent_output_deriv.Scale(opts_.chain_config.yent_regularize);
      computer->AcceptInput(yent_name, &yent_output_deriv);
      objf_info_[yent_name + suffix].UpdateStats(yent_name + suffix,
            opts_.nnet_config.print_interval,
            num_minibatches_processed_,
            yent_tot_weight, yent_tot_objf);
...
bool NnetChainAdapter::PrintTotalStats() const {
...
NnetChainAdapter::~NnetChainAdapter() {
