...
#ifndef KALDI_NNET3_NNET_CHAIN_TS_DIAGNOSTICS_H_
#define KALDI_NNET3_NNET_CHAIN_TS_DIAGNOSTICS_H_
...
#include "chain/chain-ts-training.h"
...
namespace kaldi {
    namespace nnet3 {
        ...
        struct ChainTSObjectiveInfo {
            double tot_weight;
            double tot_like;
            double tot_l2_term;
            ChainTSObjectiveInfo() : tot_weight(0.0),
                tot_like(0.0),
                tot_l2_term(0.0) { }
        };
         ...
         */
        class NnetChainTSComputeProb {
        public:
            // does not store a reference to 'config' but does store one to 'nnet'.
            NnetChainTSComputeProb(const NnetComputeProbOptions& nnet_config,
            ...
            const ChainTSObjectiveInfo* GetObjective(const std::string& output_name) const;
        ...
        private:
            ...
            unordered_map<std::string, ChainTSObjectiveInfo, StringHasher> objf_info_;
