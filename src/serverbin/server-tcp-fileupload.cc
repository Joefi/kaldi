...
int main(int argc, char* argv[]) {
    try
    {
        ...
        const char* usage =
            ...
            "\n"
            "Usage: server-tcp-fileupload [options] <save-dir>;
        ...
        while (true) {
            ...
            while (!eos)
            {
                ...
                while (true)
                {
                    ...
                    if (buffer[0] == 'N')
                    {
                        char* file_name = (buffer + 1);
                        char* save_path = save_dir.data();
                        char* file_path = strcat(save_path, file_nam);
                    ...
                    {
                        if (fwrite((buffer+1), sizeof(char), len-1, fp) < len)
                        {
