...
#ifndef KALDI_CHAIN_CHAIN_ADAPTING_H_
#define KALDI_CHAIN_CHAIN_ADAPTING_H_
...
struct ChainAdaptingOptions {
  ...
  // Cross-entropy regularization constant of phone level.  (e.g. try 0.1).  If nonzero,
  // the network is expected to have an output named 'output-yent', which
  // should have a softmax as its final nonlinearity.
  BaseFloat yent_regularize;

  ChainAdaptingOptions(): l2_regularize(0.0), out_of_range_regularize(0.01),
                          leaky_hmm_coefficient(1.0e-05),
                          xent_regularize(0.0), yent_regularize(0.0) { }
  ...
  void Register(OptionsItf *opts) {
    ...
    opts->Register("yent-regularize", &yent_regularize, "Cross-entropy "
                   "regularization constant of phone level for 'chain' training.  If "
                   "nonzero, the network is expected to have an output "
                   "named 'output-yent', which should have a softmax as "
                   "its final nonlinearity.");
...
*/
void ComputeChainObjfAndDeriv(const ChainAdaptingOptions &opts,
