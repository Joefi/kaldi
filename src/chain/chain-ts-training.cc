...
#include "chain/chain-ts-numerator.h"
...
namespace kaldi {
    namespace chain {
        ...
        void ComputeChainObjfAndDerivE2e(const ChainTSTrainingOptions& opts,
            ...
            *weight = supervision.weight * supervision.num_sequences *
            ...
            { // Doing the denominator first helps to reduce the maximum
              ...
              // we've freed the memory in this object.
                TSDenominatorComputation denominator(opts, den_graph,
        ...
        void ComputeChainObjfAndDeriv(const ChainTSTrainingOptions& opts,
            const DenominatorGraph& den_graph,
            const Supervision& supervision,
            const CuMatrixBase<BaseFloat>& nnet_output,
            BaseFloat* objf,
            ...
            { // Doing the denominator first helps to reduce the maximum
              ...
              // we've freed the memory in this object.
                ...
                den_logprob_weighted = supervision.weight * (opts.mmi_factor + opts.kl_factor) * denominator.Forward();
                if (nnet_output_deriv)
            ...
            if (opts.mmi_factor > 0.0) {
                TSNumeratorComputation numerator(supervision, nnet_output);
                // note: supervision.weight is included as a factor in the derivative from
                // the numerator object, as well as the returned logprob.
