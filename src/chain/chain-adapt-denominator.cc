...
#include "chain/chain-adapt-denominator.h"
...
AdaptDenominatorComputation::AdaptDenominatorComputation(
    const ChainAdaptingOptions &opts,
...
void AdaptDenominatorComputation::AlphaFirstFrame() {
...
void AdaptDenominatorComputation::AlphaGeneralFrame(int32 t) {
  NVTX_RANGE(__func__);
...
void AdaptDenominatorComputation::AlphaDash(int32 t) {
  NVTX_RANGE(__func__);
  BaseFloat *this_alpha = alpha_.RowData(t);
...
void AdaptDenominatorComputation::Beta(int32 t) {
  NVTX_RANGE(__func__);
...
BaseFloat AdaptDenominatorComputation::Forward() {
  NVTX_RANGE(__func__);
  AlphaFirstFrame();
  AlphaDash(0);
...
BaseFloat AdaptDenominatorComputation::ComputeTotLogLike() {
  NVTX_RANGE(__func__);
...
bool AdaptDenominatorComputation::Backward(
...
void AdaptDenominatorComputation::BetaDashLastFrame() {
...
void AdaptDenominatorComputation::BetaDashGeneralFrame(int32 t) {
  NVTX_RANGE(__func__);
  KALDI_ASSERT(t >= 0 && t < frames_per_sequence_);
  int32 num_pdfs = exp_nnet_output_transposed_.NumRows();
...
void AdaptDenominatorComputation::BetaGeneralFrameDebug(int32 t) {
