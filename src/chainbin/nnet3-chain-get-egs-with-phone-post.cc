...
static bool ProcessFile(const TransitionModel *trans_mdl,
  ...
  for (size_t c = 0; c < chunks.size(); c++) {
    ...
    Posterior phone_labels(num_frames_subsampled);
    for(int32 i = 0; i < num_frames_subsampled; i++) {
    	int32 t = (i + start_frame_subsampled) * frame_subsampling_factor;
    	if (t < phone_post.size()) {
    	    phone_labels[i] = phone_post[t];
    	}
    	for (std::vector<std::pair<int32, BaseFloat> >::iterator
                 iter = phone_labels[i].begin(); iter != phone_labels[i].end(); ++iter)
                iter->second *= chunk.output_weights[i];
    }
    SparseMatrix<BaseFloat> smat(num_phones, phone_labels);
    supervision_part.phone_post = smat;
...
int main(int argc, char *argv[]) {
  try {
    ...
    po.Register("num-phones", &num_phones, "The number of phones");
    ...
    if (po.NumArgs() == 4) {
      ...
      phone_post_rspecifier = po.GetArg(4);
    ...
    RandomAccessBaseFloatVectorReader deriv_weights_reader(
        deriv_weights_rspecifier);
    RandomAccessPosteriorReader phone_post_reader(phone_post_rspecifier);
    ...
    for (; !feat_reader.Done(); feat_reader.Next()) {
      ...
      if (!supervision_reader.HasKey(key)) {
        ...
        const Posterior& phone_post = phone_post_reader.Value(key);

        if (!ProcessFile(trans_mdl_ptr, normalization_fst, feats,
                         online_ivector_feats, online_ivector_period,
                         supervision, deriv_weights, phone_post, num_phones, supervision_length_tolerance,
